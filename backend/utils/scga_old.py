...
def read_SCGA_openpyxl(scga_f, info=None):
    """_summary_
...
def parser_SCGA(app, scga_filepath, info=None):
    ...
    if scga_filepath.endswith('xlsm') and '~' not in str(scga_filepath) and 'SCGA' in str(scga_filepath):
        output_log(f'='*80)
        output_log(f"extracting {scga_filepath}...")
        # write down frist
        scga_log_f.flush()
...
def read_SCGA(app, scga_path, info=None):
    """_summary_
...
def parser_SCGAs(app, scga_rootpath, info=None):
